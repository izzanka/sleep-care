<div>
    @foreach($therapySchedules as $schedule)
        <div class="relative rounded-lg px-6 py-4 bg-white border dark:bg-zinc-700 mb-5 dark:border-transparent"
            ...
            <div class="mt-5">
                <flux:input value="{{$schedule->link}}" readonly/>
            </div>
            <div class="flex items-center gap-2 mt-4">
{{--                <flux:icon.clock></flux:icon.clock>--}}
                <flux:heading>{{$schedule->date->isoFormat('D MMMM Y')}} ({{\Carbon\Carbon::parse($schedule->time)->format('H:i')}} - {{\Carbon\Carbon::parse($schedule->time)->addHour()->format('H:i')}})</flux:heading>
            </div>
            <flux:separator class="mt-4 mb-4"></flux:separator>
            <flux:heading>Panduan:</flux:heading>
            ...
            <flux:heading>Catatan hasil sesi terapi:</flux:heading>
